    ...
    switch (commandText) {
      case 'help':
        response = [
          ...
          '  achievements  - View certifications and honors',
          '  contact       - Reach out via email or social profiles',
          '  clear         - Clear the terminal screen',
          '  hack          - Launch a diagnostic test sequence :)'
        ];
        break;
      case 'about':
        const bioText = Array.isArray(portfolioData.personalInfo.bio) 
          ? portfolioData.personalInfo.bio.join('\n\n') 
      ...
      case 'projects':
        ...
        portfolioData.projects.forEach(p => {
          ...
          response.push(`    Description: ${p.description}`);
          response.push(`    Challenge:   ${p.challenge}`);
          
          // Handle solution formatting (could be an array or string)
          if (Array.isArray(p.solution)) {
            response.push('    Solution:');
            p.solution.forEach(sol => {
              response.push(`      - ${sol}`);
            });
          } else {
      ...
      case 'achievements':
      case 'cocurricular':
        response = ['Co-Curricular & Achievements:'];
        if (portfolioData.achievements && portfolioData.achievements.length > 0) {
          portfolioData.achievements.forEach(ach => {
            response.push(`  • [${ach.year}] ${ach.title} (Issued by: ${ach.issuer})`);
          });
        } else {
          response.push('  No achievements listed.');
        }
        break;
      case 'contact':
        response = [
